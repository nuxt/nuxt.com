...
<template>
  <div class="w-full h-full relative flex flex-col justify-center items-center bg-[#020420] py-8">
    <img
    ...
    <div class="relative w-full h-full mx-auto max-w-[1000px] flex flex-row items-center justify-between">
      <div class="flex flex-col">
        <p v-if="headline" class="text-2xl text-green-400 mb-4 font-semibold">
          {{ headline }}
        </p>
        <h1 class="text-6xl font-semibold text-white mb-4 max-w-[850px]">
          {{ title }}
        </h1>
        <p v-if="description" class="text-3xl leading-relaxed text-slate-400 max-w-[800px]" :style="{ lineClamp: 2, textOverflow: 'ellipsis' }">
          {{ description }}
        </p>
      </div>
      <div class="flex justify-center">
        <img v-if="icon" :src="moduleImage(icon)" :alt="title" class="w-40 h-40">
      </div>
    </div>
