...
// Greeting depends on `Date().getHours()` — the timezone-dependent hour can
// flip between SSR (UTC) and the browser, causing hydration mismatches.
// Compute the time-aware part client-side; the user-aware part is stable
// across server and client (driven by the session cookie).
...
async function createChat(prompt: string) {
  ...
  try {
    ...
    // Component unmounts on successful navigation; this only matters if the
    // fetch or navigation throws (or the view transition is aborted).
    loading.value = false
  }
}
