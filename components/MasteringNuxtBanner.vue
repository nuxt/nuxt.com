...
const firstEndDate = +(new Date('2023-03-23T00:00:00Z')) - Date.now()
const secondEndDate = +(new Date('2023-03-25T00:00:00Z')) - Date.now()
  ...
  <div class="relative w-full bg-white dark:bg-black z-50 border-b border-b-gray-300 dark:border-b-gray-700 mn-banner">
    <div class="flex flex-wrap sm:flex-row justify-start sm:justify-center items-center gap-x-1.5 p-2">
      <p class="text-xs sm:text-sm text-left sm:text-center items-center text-black dark:text-white justify-center items-center pl-[10px] sm:pl-0 pr-10 sm:pr-0">
        <span v-if="endDate > 0 && firstEndDate < 0" class="uppercase font-semibold mr-2">[ Extended ]</span>👨‍🏫 The <a href="https://masteringnuxt.com/?utm_source=nuxt&utm_medium=website&utm_campaign=affiliate&utm_content=top_banner&friend=nuxt" target="_blank" class="font-bold underline">Mastering Nuxt 3</a> course is now completed!
      </p>
