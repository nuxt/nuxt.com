        ...
        <li v-for="(year, index) in 11" :key="index" :class="[{'self-start': [1, 5, 8].includes(index) }, {'self-end': [3, 6, 10].includes(index) }]">
          <div
            ...
            :class="[currentStep >= index && currentStep !== null ? 'opacity-1' : 'opacity-0',
                     index === 2 ? 'ml-[27px]' : index === 4 ? 'ml-[46px]' : index === 7 ? 'ml-[42px]' : 'ml-[50px]']"
          >
          ...
          >
            <div
              class="w-1 h-0 transition-all duration-1000"
              :class="[
                {'h-[198px]': currentStep >= index && currentStep !== null },
                {'bg-gradient-to-t translate-y-1': [1, 5, 8].includes(index) },
                {'bg-gradient-to-b -translate-y-1 ': [3, 6, 10].includes(index) } ,
            ...
            <svg
              ...
              :class="currentStep >= index ? 'opacity-1' : 'opacity-0'"
            >
              <path
...
const observerCallback = (entries: IntersectionObserverEntry[]) =>
  entries.forEach((entry) => {
    if (entry.isIntersecting) {
      ...
      setTimeout(() => {
        startStepper([300, 300, 100, 100, 100, 100, 100, 100, 100, 100, 100])
      }, 600)
    }
  })
...
watch(currentStep, () => console.log(currentStep.value))
