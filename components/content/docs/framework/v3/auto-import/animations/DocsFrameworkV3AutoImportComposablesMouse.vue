<template>
  <div class="flex flex-col transition-opacity duration-300 gap-y-12" :class="currentStep > 6 && currentStep < 14 || stepsSection.includes(currentSection) ? 'opacity-100' : 'opacity-0'">
    <div class="text-gray-300 font-mono text-sm flex-col gap-y-0.5">
      <div>
        <span>{{ '<' }}</span><span class="text-gray-500">{{ 'template' }}</span><span>{{ '>' }}</span>
      </div>
      <div class="pl-4">
        <span><span>{{ '<' }}</span><span class="text-gray-500">{{ 'p' }}</span><span>{{ '>' }}</span></span>
        <span>Mouse position:
    ...
    <div class="text-gray-300 font-mono text-sm flex-col gap-y-0.5">
      <div>
        <span>{{ '<' }}</span><span class="text-gray-500">script </span><span class="text-green-400">setup</span><span>{{ '>' }}</span>
      </div>
      <div class="relative p-4 ml-4 transition-opacity duration-300 rounded-lg" :class="[{ 'opacity-0': currentStep > 2}, [1, 2].includes(currentStep) ? 'bg-gray-900 z-10 opacity-100' : 'bg-gray-800 dark:bg-gray-900']">
