  ...
  <li ref="root" class="rounded-md relative" :class="{ 'cursor-pointer': to }" @click="onClick">
    ...
    >
      <template v-if="image" #header>
        <img :src="`/assets/home/${image.dark}`" :alt="`${image} image`" class="h-full rounded-md hidden dark:block" width="350">
        <img :src="`/assets/home/${image.light}`" :alt="`${image} image`" class="h-full rounded-md dark:hidden" width="350">
      </template>
