...
<template>
  <ul :class="['platinum', 'silver', 'gold'].includes(sponsorType) ? 'grid grid-cols-3 gap-12' : 'flex gap-5 flex-wrap px-40'">
    <li v-for="{ sponsorEntity } in sponsors" :key="sponsorEntity.login">
      <a :href="`https://github.com/${sponsorEntity.login}`" :title="sponsorEntity.name" target="_blank" rel="noopener noreferrer">
        <UCard v-if="['platinum', 'silver', 'gold'].includes(sponsorType)" class="w-72" background-class="hover:u-bg-gray-50 transition-colors duration-200">
          <template #header>
            <div class="flex justify-center items-center">
              <UAvatar :rounded="sponsorType === 'silver'" :size="sponsorType === 'silver' ? 'md' : sponsorType === 'gold' ? 'lg' : 'xl'" :src="sponsorEntity.avatarUrl" />
            </div>
          </template>
          <div class="text-center truncate font-semibold text-xl">
            {{ sponsorEntity.name }}
          </div>
        </UCard>
