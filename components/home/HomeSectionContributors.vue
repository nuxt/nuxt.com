...
onMounted(async () => {
  ...
  _contributors = await $fetch('https://api.nuxt.com/contributors').then(data => data.filter(username => !['codecov-io'].includes(username)).slice(0, total * 10).map(c => c.username))
