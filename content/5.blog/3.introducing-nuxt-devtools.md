...
authors:
  - name: Anthony Fu
    ...
    link: https://twitter.com/antfu
date: 2023-03-27
category: Release
---

We announced the preview of [Nuxt DevTools](https://github.com/nuxt/devtools) on [Vue Amsterdam 2023](https://vuejs.amsterdam/), a new tool to help you understand your Nuxt app and improve the developer experience even further. And today we released a new minor version `v0.3.0` with a bunch of updates and improvements. In this post, we will explore the reasons behind the creation of Nuxt DevTools, how it can enhance your development experience, and what you can expect in the future.

## Developer Experience

Over the recent years, there has been an increasing focus on Developer Experience (DX). Tools and frameworks have been striving to improve the DX for developers. Along the way, Nuxt introduced many innovative features and conventions to make your day-to-day development easier and more efficient.

In Nuxt 3, we switched to Vite as the default bundler for the instant hot module replacement (HMR) during developement, creating a great feedback loop to your workflow. Additionally, we have introduced [Nitro](https://github.com/unjs/nitro), a new server engine that allows you to deploy your Nuxt app to any hosting service, such as Vercel, Netlify, Cloudflare, etc. **with zero-config** required..

Nuxt offers many common practices built-in. . TypeScript and ESM are available out-of-the-box throughout your codebase. Nuxt makes it simple to build single-page applications (SPA), server-side rendering (SSR), static site generation (SSG), or [hybrid them **per routes**](https://nuxt.com/docs/guide/concepts/rendering#route-rules) - using the same codebase isomorphically without any explicit setup. Nuxt also provides several composable utilities, like `useState` and `useAsyncData`, and SEO utilities, like `useHead` and `useSeoMeta`, to make states accessible across the server and client sides. Moreover, features such as the layout system, plugins, route middlewares, and other tools make app creation easier and codebases more organized.

Conventions like [file-based routing](https://nuxt.com/docs/guide/directory-structure/pages) and [file-based server APIs](https://nitro.unjs.io/guide/introduction/routing) making the routing intuitive and effortless. [Components auto-imports](https://nuxt.com/docs/guide/directory-structure/components) makes it easy to create shared components that directly available in any Vue file. Unlike global components, they are well code-splitted. We also introduced [compostables auto-import](https://nuxt.com/docs/guide/concepts/auto-imports), where all APIs from Vue are directly available. 3rd-party modules could also provide their custom composables to be auto-imported, as well as your [local composables](https://nuxt.com/docs/guide/directory-structure/composables).

Recently, we have introduced client and server-only components, which can be used by adding .client and .server to the filename with ease. All these conventions are fully typed, and developers can even have type autocomplete when doing route navigation or fetching data from APIs. These conventions significantly reduce boilerplate code, avoid duplications, and improve productivity.

And when it comes to ecosystem, Nuxt has a large community of developers building modules around it, [with hundreds of high-quality modules](https://nuxt.com/modules) available for Nuxt 3. With modules, developers can easily get integrations for features they want without worrying about the configurations or best practices.

## The Problem

With so many great features and conventions we have, Nuxt is capable of creating a large scale application at ease. However there is one problem - The **Transparency**.

For every new features and conventions we introduce, we are adding a bit more abstraction to the framework. Abstractions are great things to transfer implementation complexity and make things easier and more focused. But on the other hand, abstractions could also adding extra burden for users to learn and understand. Conventions could also lead to implicitness, like where a auto-imported component is from, or how many modules is using a certain component, etc. And of course, sometimes it can also make things hard to debug.

This might be considered a trade-off of having any tools, you have to learn and understand the tool to use it effectively. Despite from improving the documentation, and providing more examples, we believe there is still a lot of room to improve the transparency.

## Introducing Nuxt DevTools

[Nuxt DevTools](https://github.com/nuxt/devtools) is a visual tool to help you understand your Nuxt app and improve the developer experience even further. It's created to provide a better transparency of Nuxt and your app, find performance bottlenecks, and even help you managing your app and configs.

Nuxt DevTools is provided as an experimental module, and provide the views right inside your app. Once installed, it will add a small icon on the bottom of your app. Clicking it will open the DevTools panel.

To install it, please refer to the [installation guide](https://devtools.nuxtjs.org/guide).

DevTools are composed with multiple tabs for different aspects of your app:

### Overview

Shows a quick overview of your app, including the Nuxt version, the pages, the components, the modules, and the plugins you are using. It also check your Nuxt version and allow you to upgrade with a single click.

![](/assets/blog/devtools/tab-overview.png)

### Pages

Pages tab shows your current routes, and provide a quick way to navigate to them. For dynamic routes, it also provide a form to fill with each params interactively. You can also use the textbox to play and test how each route is matched.

![](/assets/blog/devtools/tab-pages.png)
...
![](/assets/blog/devtools/tab-components.png)
...
![](/assets/blog/devtools/tab-components-graph-all.png)

![](/assets/blog/devtools/tab-components-graph-filtered.png)
...
![](/assets/blog/devtools/tab-components-inspector.png)
...
![](/assets/blog/devtools/tab-imports.png)
...
![](/assets/blog/devtools/tab-modules.png)
...
![](/assets/blog/devtools/tab-modules-upgrade.png)
...
![](/assets/blog/devtools/tab-assets.png)
...
![](/assets/blog/devtools/tab-plugins.png)
...
![](/assets/blog/devtools/tab-payload.png)
...
![](/assets/blog/devtools/tab-terminals.png)
...
![](/assets/blog/devtools/tab-virtual-files.png)
...
![](/assets/blog/devtools/tab-inspect.png)
...
![](/assets/blog/devtools/tab-vscode.png)
...
![](/assets/blog/devtools/tab-vueuse.png)
...
![](/assets/blog/devtools/tab-unocss.png)

[Nuxt Icon module](https://nuxt.com/modules/icon) provides a search engine for all icons available.

![](/assets/blog/devtools/tab-icones.png)

### For Module Authors

With the release of v0.3.0, we greatly increased the ability for module authors to contribute to the DevTools. Including:
...
To learn more, please check out the [Module Authors Guide](https://devtools.nuxtjs.org/module/guide).

## What to Expect Next?

This is just the beginning of the journey. We are planning to add more features to DevTools, while exploring the ways to present the data in a more intuitive way.

The goals of Nuxt DevTools are to:

- <Icon name="uil:pathfinder"></Icon> Improve transparency for conventions
- <Icon name="uil:tachometer-fast"></Icon> Inspecting performance & analysis
- <Icon name="uil:swatchbook"></Icon> Interactive & playful
- <Icon name="uil:file-bookmark-alt"></Icon> Personalized documentations
- <Icon name="uil:create-dashboard"></Icon> Manage and scaffold the apps at ease
- <Icon name="uil:lightbulb-alt"></Icon> Provide insights of improvements
- <Icon name="uil:user-check"></Icon> Make the development experience more enjoyable

---

You can check our [Project Roadmap](https://github.com/nuxt/devtools/discussions/31) and share your [Ideas & Suggestions](https://github.com/nuxt/devtools/discussions/29), helping us to make DevTools better.
