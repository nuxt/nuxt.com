...
- You have to specify all your [dynamic routes](https://nuxtjs.org/docs/features/file-system-routing#dynamic-routes) in `generate.routes`, making it harder since you don't have access to nuxt modules there.
- You cannot test the [SPA fallback](https://nuxtjs.org/docs/concepts/static-site-generation#spa-fallback) in development, the fallback is a client-only version of your Nuxt application that loads when hitting a 404 page
...
- Fallback to client-side rendering on 404, errors and redirects (see [SPA fallback](https://nuxtjs.org/docs/concepts/static-site-generation#spa-fallback))
...
If you want to exclude a bunch of routes, use the [generate.exclude](https://nuxtjs.org/docs/configuration-glossary/configuration-generate#exclude). You can keep using [generate.routes](https://nuxtjs.org/docs/configuration-glossary/configuration-generate#routes) to add extra routes that the crawler could not detect.
...
Once you statically generated your Nuxt app into `dist/`, use `nuxt start` to start a production HTTP server and serve your static app, supporting [SPA Fallback](https://nuxtjs.org/docs/concepts/static-site-generation#spa-fallback).
...
::alert{type="next"}
To learn more about how to move from @nuxtjs/dotenv to runtime config check out [this article](https://nuxtjs.org/tutorials/moving-from-nuxtjs-dotenv-to-runtime-config).
::
