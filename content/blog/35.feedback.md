...
description: How we built a privacy-first feedback widget for our documentation pages.
navigation: false
image: /assets/blog/collecting-feedback.png
authors:
  - name: Hugo Richard
    avatar:
...
- **Actionable**: Structured data to quickly identify issues
...
- Impossible to reverse engineer original data from the hash
- GDPR compliant by design (no persistent personal data)

### 3. Database persistence with conflict handling

In the database, we use Drizzle with an intelligent `UPSERT` strategy:
...
- **Updates** if the user changes their mind within the day
- **Creation** for new feedback
- **Automatic deduplication** per page and user
