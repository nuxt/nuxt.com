...
const { data, error, clear, refresh } = await useAsyncData('users', ({signal}) => $fetch('/api/users', {
  signal
}))

refresh() // will actually cancel the request (if dedupe: cancel) via the signal
refresh() // will actually cancel the request (if dedupe: cancel) via the signal
refresh()
