...
If you have custom providers using `joinWith` for parameter formatting, you'll need to update them to use the `formatter` function with `createOperationsGenerator`. See the [migration guide](/getting-started/migration#url-formatter-changes) for details.
```

### Screen sizes

Default screen sizes now match Tailwind CSS. We've removed `xs` (320px) and `xxl` (2560px). See the [migration guide](/getting-started/migration#screen-size-changes) for how to add them back if needed.
```
...
Check out our comprehensive [migration guide](/getting-started/migration) for step-by-step upgrade instructions.
...
Most apps can upgrade with no code changes. If you have custom providers, you'll need to update them to use `defineProvider` - see the [migration guide](/getting-started/migration#custom-provider-updates) for examples.
