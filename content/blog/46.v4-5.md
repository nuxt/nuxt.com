---
title: Nuxt 4.5
description: Nuxt 4.5 is our biggest release in a while. Vite 8, Rspack 2 powered by Rsbuild, experimental SSR streaming, a stable error code system, a new useLayout composable, named views, and a lot of groundwork for Nuxt 5.
...
This one I'm really happy about. Nuxt now has a **stable error code system** ([#35429](https://github.com/n¡uxt/nuxt/pull/35429)) using [nostics](https://nostics.dev/). Warnings and errors raised during build and at runtime now carry a stable code (like `NUXT_E1001` or `NUXT_B5001`), a short explanation of **why** it happened, and a concrete **fix** to try.

Every code is greppable and bookmarkable, and the ones that need more than a one-line fix link straight to a dedicated docs page. For example, the classic "a composable was called outside a Nuxt context" now surfaces as `NUXT_E1001` with the why/fix inline and a [docs page](/docs/errors/e1001) explaining the context rules and how to use `runWithContext()`.
...
::note
This option currently applies to `useAsyncData`. Support in `useFetch` is [tracked separately](https://github.com/nuxt/nuxt/issues/29788); for now, reach for `useAsyncData` with `$fetch` when you need to gate a request.
::
