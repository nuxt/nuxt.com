    ...
    'session.started': async (_event, ctx) => {
      ...
      return {
        read_slack_channel_history: defineTool({
          description: 'Read recent messages from a Slack channel by name (e.g. firehose-nuxt, #help-nuxt) or channel ID. Returns text, permalink (Slack archive link), links (all extracted URLs), and tweetUrls (post URLs: https://x.com/<handle>/status/<id> or t.co short links — use these verbatim for "view on X", never profile URLs) per message.',
