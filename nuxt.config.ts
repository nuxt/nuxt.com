...
export default defineNuxtConfig({
  ...
  nitro: {
    typescript: {
      tsConfig: {
        // Generated by `comark-content prepare` (postinstall)
  ...
  typescript: {
    strict: false,
    tsConfig: {
      // comark-content.d.ts is generated by `comark-content prepare` (postinstall)
      include: ['../test/nuxt', '../comark-content.d.ts']
    }
  },

  // MarkdownDocument resolves custom tags against the app's global components
  // (PascalCase), so `~/components/content` must be registered as global. Add
  // it via the hook (like `@comark/nuxt` does for `components/prose`) instead
  // of overriding `components.dirs`, which would drop Nuxt's own defaults
  // (`components/global`, `components/islands`) for this layer.
  hooks: {
    'components:dirs': (dirs) => {
      dirs.unshift({ path: resolve('./app/components/content'), global: true, pathPrefix: false })
    }
  },
  eslint: {
    config: {
