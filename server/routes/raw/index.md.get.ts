...
export default defineCachedEventHandler(async (event) => {
  ...
  const title = index?.data.hero?.title?.replace(/\s+/g, ' ').trim() || 'Nuxt'
  const description = index?.data.hero?.description?.replace(/\s+/g, ' ').trim()
    || 'The Intuitive Vue Framework. Build performant and production-grade full-stack web apps and websites with confidence.'

  const featureBullets = (index?.data.features?.features ?? [])
