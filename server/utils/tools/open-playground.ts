...
export const openPlaygroundTool = tool({
  ...
  inputSchema: z.object({
    repo: z.string().regex(/^[^/\s]+\/[^/\s]+$/).describe('GitHub repository in "owner/repo" format (e.g., "nuxt/starter", "nuxt-ui-templates/dashboard")'),
  ...
  execute: async ({ repo, branch, dir, file, title }) => {
    ...
    const url = `https://stackblitz.com/github/${repo}${dirPath}?file=${encodeURIComponent(file)}`
