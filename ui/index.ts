...
const button = {
  ...
  variant: {
    ...
    ...safeColors.reduce((acc: any, color) => {
      acc[color] = `shadow-sm border border-transparent text-white bg-${color}-500 hover:bg-${color}-400 disabled:bg-${color}-500 focus:ring-2 focus:ring-offset-2 focus:ring-${color}-400`
      return acc
    }, {})
  }
}
